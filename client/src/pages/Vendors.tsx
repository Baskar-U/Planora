...
import { useVendors, useVendorCount } from "@/hooks/useFirebaseData";
...
export default function Vendors() {
  ...
  // Fetch all vendors and apply filters client-side
  const { data: allVendors = [], isLoading } = useVendors();
  const { data: totalVendors = 0 } = useVendorCount();
  
  // Type the vendors data
  ...
  const availableCities = useMemo(() => {
    ...
    vendors.forEach((vendor: any) => {
      if (vendor.location) {
        cities.add(vendor.location);
      }
    });
    return ["All", ...Array.from(cities).sort()];
  }, [vendors]);

  // Get unique categories from vendors for dynamic category list
  ...
  const stats = useMemo(() => {
    const totalServices = vendors.reduce((sum: number, vendor: any) => sum + (vendor.menu?.length || 0), 0);
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="bg-white border-b">
        <div className="w-full px-4 sm:px-6 lg:px-8 py-8">
          ...
          {/* Statistics Cards */}
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 mb-6 max-w-4xl ml-auto mr-8">
            <Card>
              ...
              <CardContent className="p-4">
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <p className="text-xl font-bold text-gray-900">4.1/5</p>
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          <div className="flex justify-center mb-4 ml-8">
            <div className="flex bg-gray-100 rounded-lg p-1">
              ...
              <Button
                variant={displayMode === "districts" ? "default" : "ghost"}
                size="sm"
        ...
        {displayMode === "districts" ? (
          ...
          <>
            {filteredVendors.length === 0 ? (
              ...
              {/* Insert ads between vendor sections */}
              {filteredVendors.length > 0 && (
                <>
                  {/* Ad after first 6 vendors */}
                  ...
                  {/* Ad after first 18 vendors */}
                  {filteredVendors.length > 18 && (
                    <div className="my-8">
                      <HorizontalAd />
                    </div>
                  )}
