...
export const useUpdateOrderStatus = () => {
  ...
  return useMutation({
    mutationFn: ({ orderId, status, timelineEntry, paymentStatus }: { orderId: string; status: string; timelineEntry: any; paymentStatus?: string }) =>
      firebaseService.updateOrderStatus(orderId, status, timelineEntry, paymentStatus),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["orders"] });
    },
  });
